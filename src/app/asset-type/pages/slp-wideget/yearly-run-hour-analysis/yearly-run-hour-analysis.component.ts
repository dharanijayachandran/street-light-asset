...
@Component({
  selector: 'app-yearly-run-hour-analysis',
  templateUrl: './yearly-run-hour-analysis.component.html',
  styleUrls: ['./yearly-run-hour-analysis.component.css']
})
export class YearlyRunHourAnalysisComponent implements OnInit {

  constructor(private assetStatusService: AssetStatusService) { }
  @Input('slpId') assetId;
  showLoaderImage: boolean = true;
  yearlyRunHour: RunHourAnalysis[];
  tooltip: Object;
  chartArea: Object;
  palette: string[];
  ...
  getYearlyRunHourAnalysis() {
    let organizationId = sessionStorage.getItem("beId");
    let timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    this.assetStatusService.getYearlyRunHourAnalysis(Number(organizationId), Number(this.assetId), String(timezone)).subscribe(responseData => {
      this.showLoaderImage = false;
      if (null != responseData) {
        this.yearlyRunHour = responseData;
